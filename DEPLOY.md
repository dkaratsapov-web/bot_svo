...
Claude Code / вы  ──push в main──►  GitHub Actions ──►  GHCR (образ)
                                                            │
                                     systemd-таймер на сервере (каждые 5 мин)
...
|---|---|
| Код бота (Python) | Actions собирает образ → таймер делает `compose pull` + `up -d` |
| compose-файлы, скрипты, Caddyfile | тот же таймер: `git pull` + `up -d` |
...
Всё делает один механизм — `scripts/selfupdate.sh` по systemd-таймеру.
Watchtower из схемы убран: его образ `containrrr/watchtower` не поддерживается
и обращается к Docker по API v1.25, который современный демон отклоняет
(`client version 1.25 is too old`). Вдобавок он требовал монтировать
Docker-сокет в контейнер, а это фактически права root на хосте.
...
3. Тариф: подойдёт `HP C1-M1-D10` (**1 vCPU / 1 ГБ / 10 ГБ**) — стек в него
   укладывается (бот ~150 МБ + Redis + Docker + ОС ≈ 400–600 МБ)
...
- **Обновления не приезжают.** Проверьте таймер и его лог:
  `systemctl status bot-svo-selfupdate.timer` и
  `journalctl -u bot-svo-selfupdate -n 50`. Обновить немедленно:
  `/opt/bot_svo/scripts/selfupdate.sh`. Убедитесь, что пакет GHCR публичный.
